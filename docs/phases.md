...
## Phase 13 — IT helpdesk (stage 2 of 4: Microsoft 365 e-mail in and out) ✅

### What works

- **Microsoft Graph connector** (`src/connectors/m365/`): app-only client credentials with a client secret or a certificate (RS256 client assertion with `x5t`), reads and writes only under `/users/{mailbox}`, immutable ids on every read, delta queries, subscriptions, draft → attach → send (small attachments inline, large ones through an upload session), `createReply`/`createReplyAll` for threading; an in-memory demo double with the same interface for tests and demo mode.
- **Mailbox administration** (Settings → Helpdesk → Support mailbox): connect/verify/reconnect/disconnect, connection facts (tenant, mailbox, credential type and expiry, permissions, last inbound sync, last notification, last outbound accepted, subscription state and renewal, webhook URL, folders, import cutoff), test connection, controlled test e-mail to a CRM user, sync now, process queues, renew subscription, behaviour settings (folders, cutoff, acknowledgement template, unknown-sender policy, closed-ticket reply policy, signature), inbound queue and outbox tables with replay / retry / cancel, sync history and errors.
- **Inbound**: durable queue (`mailbox_inbound_queue`, unique per mailbox + message id) fed by validated change notifications (`/api/webhooks/m365`, clientState checked, 202 at once) and by the 5-minute delta sync (expired sync state handled); `SKIP LOCKED` claiming; threading by `In-Reply-To`/`References`, then conversation id and `[IT-000123]` with participant validation; unauthorised or ambiguous matches become review tickets with no content disclosed; sender matched to contacts by address only; one acknowledgement per ticket with `X-Auto-Response-Suppress`, never to automated mail, rate-limited per sender; bounces tied to the outbox and flagged; auto-replies, out-of-office and bulk mail recorded as automated and never reopen or acknowledge; replies reopen resolved tickets; closed tickets reopen within a window or spawn a linked follow-up; attachments stored privately with policy checks, hashing and optional ClamAV; HTML sanitised and rendered in a sandboxed iframe with remote images blocked and quoted history collapsed.
- **Outbound**: transactional outbox with idempotency keys, honest states (queued, submitting, accepted by Microsoft 365, failed, unknown), reconciliation of timeouts against the draft id before any retry, retries with backoff, mailbox marked *expired* on auth failures, failures flagged on the ticket; reply / reply-all recipients with CC and hidden BCC, attachments uploaded and scanned first, preview before sending, send + status change in one action; compose a new e-mail (creates its ticket) from the helpdesk, a contact or a company; signature appended; Sent Items kept by Graph; activity recorded once per message.
- **Worker**: `m365.tick` every minute, `m365.delta` every 5 minutes, `m365.subscriptions` every 30 minutes.
- **Deployment**: `appdata` volume for attachments, `DATA_DIR`, attachment limits, optional ClamAV, `docs/helpdesk-m365.md` with the exact permissions, mailbox scoping (application access policy / RBAC for Applications and the additive-grant caveat), setup, monitoring, recovery and rollback.

### What was tested

- **13 unit/integration tests (145 total)** against the demo Graph double: sanitiser (scripts, handlers, iframes, remote images blocked, cid kept, `javascript:` refused), quote splitting, automated classification, attachment policy, certificate assertion and live-client URL scoping; then the acceptance scenarios: (1) one ticket, contact linked, one acknowledgement with loop header; (2) replayed notifications and concurrent workers produce no duplicates; (3, 4) a CRM reply threads (In-Reply-To), lands in Sent Items, records accepted state, repeat submit with the same key sends nothing, and the customer's reply with a changed subject joins the ticket and reopens it; (5) an unauthorised sender using a reference gets a review ticket with no content and no acknowledgement; (6) uploads are policy-checked, outbound attachments sent, and a reply to a merged ticket's old reference reaches the survivor; (7) internal notes never reach the outbox; (8) delta sync recovers a missed message, honours the import cutoff and survives an expired sync token; (9) subscription create/renew/lifecycle, forged clientState rejected, revoked permission flips the mailbox to expired and recovers, throttling retries; (10) a timed-out send is reconciled not resent, hard failures retry then surface on the ticket and an admin retry succeeds; (11) out-of-office and bounces never reopen or answer, acknowledgements are rate-limited per sender, a human reply reopens, a late reply to a closed ticket opens a linked follow-up; compose and test e-mails.
- **1 browser test (30 total)**: mailbox page in demo mode shows state and queues, a test e-mail is accepted, an agent replies by e-mail from a ticket with preview and the message shows *accepted*, and composes a new e-mail from a contact that opens a ticket.

### Remaining dependencies (live verification)

- Connect a real Microsoft 365 test mailbox and follow `docs/helpdesk-m365.md` §8. Live Graph behaviour has not been verified from this environment.
- Response templates, agent mentions and notifications arrived in stage 3.

## Phase 13 — IT helpdesk (stage 3 of 4: collaboration, SLAs, automation, reporting) ✅

### What works

- **Data model** (`src/db/schema/helpdesk-sla.ts`, migration `0014_helpdesk_sla.sql`): `helpdesk_business_hours` (weekly windows in an IANA time zone, holidays, 24×7 flag), `helpdesk_sla_policies` (first-response and resolution minutes per priority, pause statuses, default flag), `helpdesk_sla_assignments` (company → policy), `ticket_sla_events` (start / pause / resume / met / breach / reset / policy_changed per clock, with due-at and elapsed minutes), `helpdesk_automation_rules` + `helpdesk_automation_runs`, `helpdesk_templates`, `ticket_checklist_items`, `helpdesk_notifications`.
- **SLA engine** (`src/lib/helpdesk/business-hours.ts`, `src/services/helpdesk-sla.ts`): business-time arithmetic walks calendar days in the zone and converts each window with `fromZonedTime`, so daylight-saving changes move with the wall clock; holidays skipped. Both clocks are replayed from their event history on every change (create, status, priority, company, first public reply, resolve, reopen); the resolution clock pauses in the policy's pause statuses, the first-response clock never pauses; reopening resets the clocks with history kept; policy edits recompute every open ticket. The ticket page shows both clocks with elapsed/target, deadline, a progress bar and the full clock history. The worker (`helpdesk.sla`, every 5 min) flags breaches (event + notification to assignee and followers) and warns an hour before a deadline, then fires the `sla_breached` / `sla_due_soon` rule triggers.
- **Automation** (`src/services/helpdesk-automation.ts`, Settings → Helpdesk → Automation rules): triggers ticket created / updated / customer replied / agent replied / status changed / SLA breached / SLA due soon / schedule (every 15 min with an idle threshold); conditions on status, priority, type, source, category, team, assignee, tags, subject, requester e-mail/domain, unverified/review flags and hours since activity/created/resolved with is / is not / one of / contains / empty / greater / less, matched all or any; actions assign agent or team, set priority / status / category / type, add or remove tag, add internal note, notify users or the assignee, close if resolved. Rules run in order, at most once per ticket within their cooldown, can stop later rules, and changes made by a rule never re-enter the engine beyond three chained steps (so two status rules cannot ping-pong). Every run is logged with the actions applied and any error; the ticket timeline records rule actions with the rule name.
- **Hooks** (`src/services/helpdesk-hooks.ts`): every ticket change commits first, then recomputes the SLA and runs the rules; failures there are logged and never undo the user's action. Mailbox events feed the same path (customer reply → `customer_replied`, accepted send → `agent_replied`).
- **Collaboration**: `@name` mentions in notes and messages (matched against active users by name, name prefix or e-mail local part) notify the mentioned user; assignee and followers are notified of new messages and notes, customer replies, assignments, SLA warnings, bounces and checklist assignments, never the actor themselves, de-duplicated per kind and ticket within ten minutes. A bell in the header shows the unread count; Helpdesk → Notifications lists them with mark-read. **Response templates** (Settings → Helpdesk → Templates, `helpdesk.manage`) with `{{requester.first_name}}`, `{{requester.name}}`, `{{company.name}}`, `{{ticket.reference}}`, `{{ticket.subject}}`, `{{agent.name}}` are inserted from the composer, filtered by reply/note scope. **Checklists** per ticket with assignee, due date and completion; the composer warns when resolving with items still open.
- **Reporting** (Helpdesk → Reports): date range (default last 30 days) plus customer, agent, team, category and priority filters; created / resolved volumes, median and average first response and resolution, first-response and resolution SLA attainment among tickets with a target, reopen rate, time logged, backlog age (average, oldest, older than 7 days, breaching now), a created-vs-resolved daily chart, breakdowns by agent, team, category, customer, priority and type (created, resolved, median resolution, breached, time), and time logged by agent and customer. CSV exports (`/api/helpdesk/export/{tickets|sla|time|agents|companies}`, `helpdesk.manage`, audited) carry ticket fields and SLA outcomes, never message bodies. Dashboard tiles show 7-day attainment and the number breaching now.
- **Seed**: UK office hours, a default *Standard support* policy and a *Priority 24×7* policy assigned to one customer, three templates and three rules (route e-mail tickets to the service desk, outage keywords → critical, close resolved tickets after 5 days).
...
- **Monitoring** (`/api/health` gains a `helpdesk` block; Settings → Helpdesk → Operations): mailbox state and subscription, stale inbound sync, dead inbound rows, unknown/failed outbox rows, open/review/breaching counts, last SLA check, rules run and retention run; `degraded` when any needs attention. Replay and retry tools stay on the mailbox page.
