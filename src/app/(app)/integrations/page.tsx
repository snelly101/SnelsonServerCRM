...
export default async function IntegrationsPage() {
  ...
  return (
    <>
      <PageHeader
        title="Integrations"
        description="Connection status, mapping, manual sync, history and unresolved conflicts."
        actions={
          <Badge tone={worker.status === "alive" ? "green" : "red"}>
            worker{" "}
            {worker.status === "alive"
              ? `alive · seen ${fmtRelative(worker.lastSeen)}`
              : worker.status}
          </Badge>
        }
      />
      {worker.status !== "alive" && (
        <Alert
          ...
          title={
            worker.status === "never"
              ? "Background worker has never checked in"
              : "Background worker looks stopped"
          }
          className="mb-4"
        >
          {worker.lastSeen
            ? `Last heartbeat ${fmtRelative(worker.lastSeen)}. `
            : ""}
          Scheduled syncs, reminders and webhook processing will not run until{" "}
          <code>npm run worker</code> (or the cron tick) is running. Uptime
          monitors can watch <code>/api/health</code>.
        </Alert>
      )}
      {demo && (
        <Alert tone="warn" title="Demo mode is on" className="mb-4">
          Providers without credentials use synthetic data and are shown as{" "}
          <strong>Demo (not connected)</strong>. Nothing marked demo is live or
          verified.
        </Alert>
      )}
      <div className="mb-4 grid gap-3 md:grid-cols-2 xl:grid-cols-4">
        {health.connections
          .filter((c) => c.provider !== "m365")
          .map((c) => {
            const provider = c.provider as Provider;
            const isDemo =
              provider === "betterproposals"
                ? bp.demo
                : provider === "xero"
                  ? xero.demo
                  : provider === "ninjaone"
            ...
            return (
              <Card
                ...
                actions={
                  <Badge tone={isDemo ? "amber" : STATUS_TONE[c.status]}>
                    {label}
                  </Badge>
                }
              >
                    ...
                    <dd
                      title={
                        c.lastSuccessfulSyncAt
                          ? fmtDateTime(c.lastSuccessfulSyncAt, settings)
                          : ""
                      }
                    >
                      {c.lastSuccessfulSyncAt
                        ? fmtRelative(c.lastSuccessfulSyncAt)
                        : "never"}
                    </dd>
                  </div>
                  <div className="flex justify-between gap-2">
                    ...
                    <dd>
                      {c.lastTestedAt ? fmtRelative(c.lastTestedAt) : "never"}
                    </dd>
                  </div>
                ...
                <div className="mt-3 flex flex-wrap gap-2">
                  <Link
                    ...
                    className="inline-flex h-8 items-center rounded-md border border-slate-300 bg-surface px-2.5 text-xs font-medium hover:bg-slate-50"
                  >
      ...
      <div className="mb-4 grid grid-cols-2 gap-3 lg:grid-cols-4">
        <Stat
          label="Proposals mirrored"
          value={counts.total}
          hint={bp.demo ? "demo data" : undefined}
        />
        <Stat label="Awaiting signature" value={counts.sent} />
        <Stat label="Signed / paid" value={counts.signed} tone="good" />
        <Stat
          label="Unlinked proposals"
          value={counts.unlinked}
          tone={counts.unlinked ? "warn" : "default"}
          hint={counts.unlinked ? "link them on the Proposals page" : undefined}
        />
      </div>

      {conflicts.length > 0 && (
        <Card
          title={`Needs review (${conflicts.length})`}
          padded={false}
          className="mb-4"
        >
              ...
              >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="text-xs text-slate-500">
                    ...
                    {c.kind.replace("_", " ")} · {fmtRelative(c.createdAt)}
                  </div>
                </div>
                {canManage && (
                  <span className="flex shrink-0 gap-1">
                    <ConfirmButton
                      variant="ghost"
                      size="sm"
                      action={resolveConflictAction.bind(
                        null,
                        c.id,
                        "resolved",
                      )}
        ...
        {runs.length === 0 ? (
          <div className="p-4">
            <EmptyState
              ...
              title="No syncs yet"
              description="Runs appear here with counts and any errors."
            />
          </div>
        ) : (
          <table className="tbl">
              ...
              {runs.map((r) => (
                <tr key={r.id}>
                  <td className="whitespace-nowrap text-slate-500">
                    {fmtDateTime(r.startedAt, settings)}
                  </td>
                  ...
                  <td className="text-slate-600">
                    {r.trigger}
                    {r.triggeredBy && (
                      <span className="text-xs"> · {r.triggeredBy}</span>
                    )}
                  </td>
                  <td>
                    <Badge
