...
import { mappingOverview, xeroConnectionSummary, xeroReferenceData } from "@/services/xero";
...
import { TenantPicker, XeroConfigForm, XeroTestButton, XeroSyncButton, MappingTable } from "./controls";
...
export default async function XeroPage({ searchParams }: { searchParams: Promise<{ error?: string; connected?: string; choose?: string }> }) {
  ...
  const [conn, settings, runs, errors, outbound, mapping] = await Promise.all([xeroConnectionSummary(), getAppSettings(), listSyncRuns("xero", 10), recentUnresolvedErrors("xero"), listOutbound("xero", 15), mappingOverview()]);
