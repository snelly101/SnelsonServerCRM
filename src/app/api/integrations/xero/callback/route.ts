...
export async function GET(req: Request) {
  ...
  if (error || !code || !state) {
    return NextResponse.redirect(appUrl(`/integrations/xero?error=${encodeURIComponent(error ?? "Xero did not return an authorisation code")}`, req.url));
  }
  try {
    ...
    return NextResponse.redirect(appUrl(res.selected ? "/integrations/xero?connected=1" : "/integrations/xero?choose=1", req.url));
  } catch (err) {
    logger.error({ err }, "xero oauth callback failed");
    return NextResponse.redirect(appUrl(`/integrations/xero?error=${encodeURIComponent(err instanceof Error ? err.message : "Connection failed")}`, req.url));
  }
}
